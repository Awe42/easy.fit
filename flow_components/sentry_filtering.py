...
def lambda_handler(event, context):
    """
    AWS Lambda handler function for data filtering.
    
    Expected event structure:
    {
        "body": "<JSON string containing sentry input data>"
    }
    OR
    {
        "sentry_input_data": "<JSON string containing sentry input data>"
    }
    OR
    {
        "node": {
            "inputs": [{"name": "sentry_input_data", "value": "..."}]
        }
    }
    OR directly as JSON object
    
    Returns:
    - For Bedrock Flows: Returns the filtered data JSON string directly
    - For API Gateway: Returns HTTP response object (detected via event structure)
    """
    ...
    try:
        ...
        if isinstance(event, str):
            ...
            sentry_input_data = event
        elif "node" in event and "inputs" in event["node"]:
            # Bedrock Flows format
            inputs = event["node"]["inputs"]
            sentry_input_data = None
            for input_item in inputs:
                if input_item.get("name") == "sentry_input_data":
                    sentry_input_data = input_item.get("value")
                    break
            if sentry_input_data is None:
                raise ValueError("sentry_input_data not found in Bedrock Flows inputs")
        elif "body" in event:
            # API Gateway format
            sentry_input_data = event["body"]
            if isinstance(sentry_input_data, str):
                # Body might be double-encoded
                try:
                    test_parse = json.loads(sentry_input_data)
                    if isinstance(test_parse, str):
                        sentry_input_data = test_parse
                except (json.JSONDecodeError, TypeError):
                    pass
        elif "sentry_input_data" in event:
            # Direct parameter format
            sentry_input_data = event["sentry_input_data"]
        else:
            # Assume event is the data object itself
            sentry_input_data = json.dumps(event)
        
        # Parse the input JSON string
        if isinstance(sentry_input_data, str):
            data = json.loads(sentry_input_data)
        else:
            data = sentry_input_data
        ...
        error_response = {
            ...
            "message": str(e),
            "event_received": json.dumps(event)[:500]  # Include first 500 chars for debugging
        }
